...
// generated loader at build time, and `current` accumulates bytes read from the
// decompressed stream. Both are in the same units, so the ratio holds even
// though the file arrives compressed and Content-Length is a much smaller
// number.
//
// Bevy's bundle is tens of megabytes uncompressed, so this is not a nicety:
// without it the page is a spinner for however long the visitor's connection
// takes, with no way to tell progress from a hang.
