<#
.SYNOPSIS
    Build the Relief Forge Windows installer (MSI).

.DESCRIPTION
    Compiles the release binaries and packages both — the GUI and the command-line tool — into
    a single per-user MSI.
...
.PARAMETER Version
    Override the version. Defaults to the version in the workspace Cargo.toml.
...
.EXAMPLE
    .\installer\build.ps1
    Build release binaries and produce target/installer/ReliefForge-0.1.0-x64.msi

.EXAMPLE
    .\installer\build.ps1 -SkipBuild -Version 0.2.0
#>
[CmdletBinding()]
param(
    [switch]$SkipBuild,
...
# Everything the MSI ships. Named here so a missing one is a clear error before WiX is invoked.
$Binaries = @('relief-app.exe', 'relief.exe')
...
<#
    The workspace version, from [workspace.package] rather than [package].

    Every crate here says `version.workspace = true`, so the number exists in exactly one place
...
function Get-CargoVersion {
    ...
    foreach ($line in Get-Content $manifest) {
        ...
        # Stop at the next table so a dependency's version is never picked up.
        if ($inWorkspacePackage -and $line -match '^\s*\[') { break }
        if ($inWorkspacePackage -and $line -match '^\s*version\s*=\s*"([^"]+)"') { return $Matches[1] }
    }
    throw "Could not find a [workspace.package] version in $manifest"
}
...
<#
    ...
    which is Lorem ipsum. An installer that asks people to accept filler text builds, validates
    and installs perfectly well, so the real licence is generated here rather than left to a
    default.

    Generated at build time instead of committing a second copy: an RTF checked in beside
    LICENSE is a duplicate that drifts, and the whole point is that the dialog shows the licence
    the project is actually under.

    Monospace, and one \par per source line, to preserve the layout of the source text.
#>
...
<#
    ...
    a red graphic in the top-right of every page.
...
function New-UiBitmaps {
    ...
    try {
        ...
        try {
            ...
            $g.PixelOffsetMode = [System.Drawing.Drawing2D.PixelOffsetMode]::HighQuality
            $g.Clear([System.Drawing.Color]::White)
            # The icon's own background, so the rounded square sits *on* the band rather than
            # floating in a rectangle of some other dark colour. Both stops are within a shade
            # of resources/icon.png's #191c1e.
            $band = New-Object System.Drawing.Rectangle(0, 0, 164, 312)
            $top = [System.Drawing.Color]::FromArgb(34, 39, 42)     # #22272a
            $bottom = [System.Drawing.Color]::FromArgb(14, 17, 19)  # #0e1113
...
Write-Step "Relief Forge $cargoVersion (MSI version $msiVersion)"

if (-not $SkipBuild) {
    Write-Step 'Building release binaries'
    Push-Location $RepoDir
    try {
        Invoke-Tool -What 'cargo build' -Exe 'cargo' -Arguments @('build', '--release', '-p', 'relief-app', '-p', 'relief-cli')
...
foreach ($binary in $Binaries) {
    ...
    if (-not (Test-Path $path)) {
        throw "$path not found. Run without -SkipBuild, or build it first with 'cargo build --release -p relief-app -p relief-cli'."
    }
}
...
$wixObj = Join-Path $objDir 'relief-forge.wixobj'
$msi = Join-Path $OutDir "ReliefForge-$cargoVersion-x64.msi"
...
Invoke-Tool -What 'candle' -Exe (Join-Path $wixBin 'candle.exe') -Arguments @(
    ...
    '-out', $wixObj
    (Join-Path $InstallerDir 'relief-forge.wxs')
)
...
Write-Host ''
Write-Note 'Installs to %LOCALAPPDATA%\Programs\Relief Forge for the current user; no UAC prompt.'
Write-Note 'Both the app and the `relief` command-line tool; the folder goes on the user PATH.'
